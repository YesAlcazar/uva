...
def minimumBribes(q):
    ...
    for x in range(len(q)):
        if  q[x] - x > 3:
            print ('Too chaotic')
            return
    sorted(q,cmp=numericCMP)
    print q
    print(minimum)
    # for i in range(size):
    #
    #     j = complement(i)
    #     if  q[i] - i > 3 or q[j] - j > 3:
    #         print ('Too chaotic')
    #         return
    #     elif q[i] > i + 1:
    #         minimum++
...
#     for x in range(1, 5):
#         for y in range(1, 5):
#             print '%d %d %d %d %d' % (x,y,arr[x][y],sum(arr[x - 1][y - 1 : y + 2]),sum(arr[x + 1][y - 1 : y + 2]))
#             retorno.append(arr[x][y] + sum(arr[x - 1][y - 1 : y + 1]) + sum(arr[x + 1][y - 1 : y + 1]))
#     return max(retorno)
#
# arr = [
# [1, 1, 1, 0, 0, 0],
...
#     #
#     return '%02d%s' % (int(s[:2]) + 12 * (s[8:] == 'PM'),s[2:8])
#     # return str(int(s[0:1]) + 12 * (s[8:9] == 'PM')) + s[2:7]
#
# print timeConversion('07:05:45PM')

# def miniMaxSum(arr):
#     total = sum(arr)
#     print('%d %d' % (total - max(arr), total - min(arr)))
#
# miniMaxSum([1,2,3,4,5])

# Complete the compareTriplets function below.
# def compareTriplets(arr):
#     # size = len(arr) -1
#     # # abs = 0
#     # # for i in xrange(0,len(a))
#     #
#     # # return functools.reduce(lambda s,r: [s[0]+r[0],s[1]+r[1]],([x > y, x < y] for x,y in zip(a,b)))
#     # # return abs(sum(arr[i][i] - arr[i][size-i] for i in xrange(size)))
#     # print size
#     # return list(arr[i][i] - arr[i][size-i] for i in xrange(0,size+1))
#     a = b = c = 0
#     e = lambda y: y/float(len(arr))
#     d = lambda z: e(z) - e(z) % 0.000001
#     for x in arr:
#         a += (x > 0)
#         b += (x == 0)
#         c += (x < 0)
